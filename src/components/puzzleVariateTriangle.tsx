...
export default function PuzzleRandomShapes({
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        console.log('Loading puzzle with image:', imageSrc);
  ...
  // Check if piece should snap to neighbors - FIXED VERSION
  const checkAndSnapToNeighbors = useCallback((pieceId: string, position: Point) => {
    ...
    Object.values(piece.neighbors).forEach(neighborId => {
      ...
      // CRITICAL FIX: Calculate offset using correctPosition (which accounts for bounds.minX/minY)
  ...
  const handleMouseUp = useCallback(() => {
    ...
    if (placed) {
      ...
      if (shouldSnap && neighbors.length > 0) {
        // Calculate offset to apply to all dragged pieces
        ...
        setPlacedPieces(prev => {
          ...
          // Update all dragged pieces with snap offset
          dragState.pieceIds.forEach(id => {
            const old = prev.get(id);
            if (!old) return;
